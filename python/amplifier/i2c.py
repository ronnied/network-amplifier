...
class I2cInit:
  def __init__(self):
    ...
    self.BCM2708_PERI_BASE=0x20000000
    self.GPIO_BASE=(self.BCM2708_PERI_BASE + 0x00200000)
    self.BLOCK_SIZE=4096
    self.i2cConfig()

  def _strto32bit_(self, str):
    return ((ord(str[3])<<24) + (ord(str[2])<<16) + (ord(str[1])<<8) + ord(str[0]))

  def _32bittostr_(self, val):
  ...
  def i2cConfig(self):
    ...
    m = mmap.mmap(mf, self.BLOCK_SIZE, mmap.MAP_SHARED, 
    ...
    m.seek(0)
    reg0=self._strto32bit_(m.read(4))
    # GPFSEL2 -- GPIO 28,29 I2C0
    m.seek(8)
    reg2=self._strto32bit_(m.read(4))
